...
class GuardarProducto
{
    ...
    private function insertar(array $datos): array
    {
        ...
        $producto = $this->buscarPorUid($uid, $empresa);
    ...
    private function actualizar(string $uid, array $datos): array
    {
        ...
        // Verificar que el producto existe en esa empresa
        $producto = $this->buscarPorUid($uid, $empresa);
        if (!$producto) {
            return $this->respuesta(false, null, 'Producto no encontrado o no pertenece a la empresa actual.', ['uid_no_existe']);
        }
        ...
        $stmt = $this->pdo->prepare($sql);
        $stmt->execute(array_merge(
        ['uid' => $uid, 'empresa' => $empresa, 'usuario_ult_modificacion' => $usuarioModificacion],
            $this->extraerCampos($datos)
        ));
    ...
    // ── Busca un producto por UID y EMPRESA (Garantiza Aislamiento) ──
    private function buscarPorUid(string $uid, string $empresa = ''): ?array
    {
        if ($empresa === '') {
            return null; // Por seguridad, si no hay empresa, no devuelve nada
        }
